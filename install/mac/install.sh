...
CONFIG_DIR="$HOME/Library/Application Support/odoo-mcp"
...
ARCH=$(uname -m)
if [ "$ARCH" = "arm64" ]; then
    ASSET="odoo-mcp-aarch64-apple-darwin"
elif [ "$ARCH" = "x86_64" ]; then
    ASSET="odoo-mcp-x86_64-apple-darwin"
else
...
echo "────────────────────────────────────────"
echo "You can configure multiple profiles. Let's start with one."
echo ""
...
if [ ! -f "$CLAUDE_CONFIG" ]; then
    prompt "Claude Desktop config not found at:"
    echo "  $CLAUDE_CONFIG"
...
if [ -f "$CLAUDE_CONFIG" ]; then
    # Check if odoo-mcp already configured
